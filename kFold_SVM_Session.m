...
sigma_user = input('Optimized RBF Sigma Value for this subject: ');
...
for k = 1:folds
    ...
    fprintf('\n')
    disp(['SVM Train - Fold '  num2str(k) '  #Samples Train = ' num2str(N_session - (ind_change(k+1)-ind_change(k))) '  #Samples Test = ' num2str((ind_change(k+1)-ind_change(k)))]);
    ...
    warning off;
    [trainresult,testresult] = multisvmE(features(TrainingSet,:),codesTrue(TrainingSet)',features(testSet,:),'rbf',sigma_user);
    warning on;    
    
    timeSVM = toc; %end timer
      
    %SVM Prediction and SVM class probabilities for ENTIRE dataset. This is
    %for initializing the HMM Emission matrix (P_SVM(TrainSet)) and for
    %computing the observations of the HMM (P_SVM(TestSet))
    codesSVM = testresult;
