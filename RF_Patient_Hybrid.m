...
cd(fileparts(which('RF_Patient_Hybrid.m')))
currentDir = pwd;
slashdir = '/';
...
conf_folder = [ARCode 'confusion_matrices/Hybrid/'];
addpath(conf_folder);
...
%% PROCESS DATA FOR OTHER PATIENTS (impersonal data)
subject_indices2 = find(subject_analyze~=all_subjectID);
cData_ALL_temp = isolateSubject(trainingClassifierData,subject_indices2);
if strcmpi(population,'patient')
    for zz = 1:length(cData_ALL_temp.subject)
        temp = char(cData_ALL_temp.subject(zz));
        cData_ALL_temp.subjectBrace(zz) = {temp(7:9)};
    end
end
cData_ALL = isolateBrace(cData_ALL_temp,brace_analyze);

%Extract data from struct
features_other = cData_ALL.features;
subjects_other     = cData_ALL.subject;  %subject number
uniqSubjects_other = unique(subjects_other); %list of subjects
statesTrue_other = cData_ALL.activity;     %all the classifier data
subjectID_other = cData_ALL.subjectID;
sessionID_other = cData_ALL.sessionID;
...
uniqStates  = unique(statesTrue_other);          %set of states we have
...
codesTrue_other = zeros(1,length(statesTrue_other));
for i = 1:length(statesTrue_other)
    codesTrue_other(i) = find(strcmp(statesTrue_other{i},uniqStates));
end
...
%     CostM(3,:) = 5; CostM(3,3) = 0;
    ...
    RFmodel = TreeBagger(ntrees,[features_other; features(TrainingSet,:)],[codesTrue_other'; codesTrue(TrainingSet)'],'OOBVarImp',OOBVarImp,'Options',opts_ag);
