...
for k = 1:folds
    ...
    fprintf('\n')
    disp(['NB Train - Subject '  num2str(user_subjects(k)) '  #Samples Train = ' num2str(N_session - (ind_change(k+1)-ind_change(k))) '  #Samples Test = ' num2str((ind_change(k+1)-ind_change(k)))]);
...
accNB = 0; matNB = zeros(nstates); matHmm = zeros(nstates);
accNB_uw = 0;
for i = 1:folds
...
fprintf('\n')
disp(['Unweighted Mean (k-Subject) accNB = ' num2str(accNB_uw)]);
disp(['Weighted Mean (k-Subject) accNB = ' num2str(accNB)]);
