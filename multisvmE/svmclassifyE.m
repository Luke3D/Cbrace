function [outclass,sep] = svmclassifyE(svmStruct,sample, varargin)
...
%     try
        [outclass,sep] = svmdecision(sample,svmStruct);
%     catch ME
%         if strcmp(ME.identifier, 'MATLAB:nomem')
%             try %reduce data size
%                 Nsamp = size(sample,1);
%                 meminfo = memory;
%                 Nmax = meminfo.MaxPossibleArrayBytes/8; %max number of elements in hyperplane variable
%                 %Nmax = sqrt(Nmax)-1; %max elements in training set
%                 Nmax = min(floor(Nsamp/2),floor(Nmax/length(svmStruct.Alpha))-1); %max elements in training set
%                 loopflag = true;
%                 n = 1;
%                 outclass = zeros(1,Nsamp);
%                 sep = zeros(1,Nsamp);
%                 while loopflag
%                     if n + Nmax > Nsamp
%                         idx = n:Nsamp;
...
%                         idx = n:(n+Nmax);
%                     end
%                     try %try SVM with reduced dataset
%                         [outclass(idx),sep(idx)] = svmdecision(sample(idx,:),svmStruct);
%                         n = n+Nmax+1;
%                     catch ME %further reduce dataset
%                         if strcmp(ME.identifier, 'MATLAB:nomem')
%                             Nmax = floor(Nmax/2);
%                             n = 1;
%                             loopflag = true;
...
%                     end
%                 end
%                 %Nmax = floor(Nmax/size(TrainingSet,2)); %max length of training set
%             catch ME
