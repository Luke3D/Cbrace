...
%Do k fold cross validation for NB
for k = 1:folds
    ...
    fprintf('\n')
    disp(['NB Train - Fold '  num2str(k) '  #Samples Train = ' num2str(N_session - (ind_change(k+1)-ind_change(k))) '  #Samples Test = ' num2str((ind_change(k+1)-ind_change(k)))]);
    ...
    warning off;
    NBmodel = fitcnb(features(TrainingSet,:),codesTrue(TrainingSet)');
    testresult = predict(NBmodel,features(testSet,:));
    warning on;    
    
    timeNB = toc; %end timer
      
    %NB Prediction and NB class probabilities for ENTIRE dataset. This is
    %for initializing the HMM Emission matrix (P_NB(TrainSet)) and for
    %computing the observations of the HMM (P_NB(TestSet))
    codesNB = testresult;
...
acc_tbl = table(test_size,train_size,acc_NB_vector,'RowNames',row_folds,'VariableNames',{'Test_Size','Train_Size','NB_Acc'});
fprintf('\n')
disp(acc_tbl)
