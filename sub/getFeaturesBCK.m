function [fvec flab] = getFeatures(pre_acc, varargin)
...
%
% INPUTS
% pre_acc:  a 4 x samples vector: 1 is time, 2-4 are xyz accelerations
%      pre is for prior to interpolation
% secs: the number of seconds to extract
% rate: the rate to interpolate

if length(varargin) == 0
    secs = 10; % take the final block of 'secs'
else
    secs = varargin{1};
end

if length(varargin) <= 1
    rate = 10; % hz to interpolate to
else
    rate = varargin{2};
end

% extract the relevant time in the acceleration sample
max_sec = max(pre_acc(1,:));
start_sec = max_sec - secs;

% perform the interpolation
arr_size = round(secs * rate);
acc = zeros([4, arr_size]);

acc(1,:) = start_sec + ([0:arr_size-1]' / arr_size * secs);

for dim = 2:4
    acc(dim,:) = interp1(pre_acc(1,:), pre_acc(dim,:), acc(1,:), 'linear', 'extrap');
end
...
% % now projecting the vector along gravity and perpendicular to it
% g_vec = mean(S');
% g_dir = g_vec / norm(g_vec);
% vert_comp = g_dir * S;
% horiz_comp = S - g_dir' * vert_comp;
% horiz_mag = zeros(1,size(horiz_comp,2));
% for i = 1:length(horiz_mag)
%   horiz_mag(1,i) = norm(horiz_comp(:,i));
% end
% S = vertcat(vert_comp, horiz_mag);
...
for i=1:size(S,1)
    ...
    fvec = [fvec Pyy];
    flab = [flab; cellstr(repmat('fourier',8,1))];
