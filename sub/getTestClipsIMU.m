function [accelerations, date, activities, activity_fraction] = getTestClipsIMU(accdata, varargin)
...
for file = 1:length(datafiles)
    ...
    while currentRow < num_rows
        ...
        if elapsedTime - options.secs > 0
            ...
            %extract time, acc and gyro data (7 rows)
            acc = zeros(7,currentRow-startInd+1);
            for d = 1:7
