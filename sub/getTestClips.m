function [accelerations, date, activities, activity_fraction, num_clips] = getTestClips(accdata, varargin)
...
num_clips = zeros(length(datafiles),1); %number of clips from each csv file
...
for file = 1:length(datafiles)
    ...
    while currentRow < num_rows
        ...
        if elapsedTime - options.secs > 0
            ...
            currentRow = currentRow - floor((endInd-startInd)*options.overlap);
            startTime = data{1}(currentRow);
